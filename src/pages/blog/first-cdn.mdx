...
If you've been living under a rock, MoQ is an [up-and-coming standard](https://datatracker.ietf.org/group/moq/about/) for live media, aiming to supplant [WebRTC](./replacing-webrtc), [HLS/DASH](./replacing-hls-dash), and even **RTMP/SRT** as the one to rule them all.
...
Also, *while you're here*, some shameless self-promotion: I just soft-launched [hang.live](./first-app).
...
[hang.live](./first-app) uses the far more powerful Javascript API to do more complicated stuff like get access to individual video frames.
...
(and yes, [I'm aware that WASM exists](./to-wasm), but I ended up abandoning it)
...
It should even work on private networks provided you [wrestle with TLS certificates](./tls-and-quic).
...
It powers [hang.live](./first-app) so the API is subject to change and is probably already out of date.
